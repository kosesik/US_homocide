...
hist(Victim_Age)
hist(Perpetrator_Age)

```

For ease of analysis, let's now turn numerical variables into categrical ones.
...
Proportions of binary responses of target variable have been maintained in the training and testing sets. Now, we need to deal with high imbalance of the training dataset. To that end, SMOTE algoritm will be applied.


```{r}

levs<-sapply(train_set,levels)

train_set<-sapply(train_set,as.numeric)

train_set<-data.frame(train_set)

temp<-sum(train_set$Crime_Solved==2)/sum(train_set$Crime_Solved==1)

train_set<-SMOTE(train_set[,-3],train_set[,3],K = 3,dup_size = temp)
train_set<-train_set$data
table(train_set$class)

ggplot(train_set,aes(x=class))+geom_bar(fill="sky blue")+labs(x="Crime_Solved")+scale_y_continuous(labels=comma)+theme_bw()

train_set$class<-as.numeric(train_set$class)
train_set<-round(train_set)


train_set$Agency_Type<-as.factor(train_set$Agency_Type)

train_set<-lapply(train_set, as.factor)
...
The data is however highly imbalanced. Unsolved cases are only 1/1000 of the dataset.

```{r}

round(prop.table(table(Crime_Solved)),3)*100

temp<-sum(Crime_Solved=="Yes")/nrow(data)

cat("The number of cases with Crime_Solved variable as 'No' is equal to", temp)
```


In case of such a highly imbalanced data, we need to employ mechanism to construct a more balanced dataset to ensure reliability of the model results. We will use "SMOTE" algorithm to that end.

```{r}

data_adj<-sapply(data,as.numeric)

class(data_adj)
data_adj<-data.frame(data_adj)
class(data_adj)

data_adj$Crime_Solved<-data_adj$Crime_Solved-1

temp<-sum(data_adj$Crime_Solved==1)/sum(data_adj$Crime_Solved==0)

data_adj<-SMOTE(data_adj[,-3],data_adj[,3],K = 3,dup_size = temp)
data_adj<-data_adj$data
table(data_adj$class)

ggplot(data_adj,aes(x=class))+geom_bar(fill="sky blue")+labs(x="Crime_Solved")+scale_y_continuous(labels=comma)+theme_bw()
...
Let's make sure the balance of train and test sets is maintained.
...
``{r}
model_log<-train(class~.,data=train_set, method="glm", family="binomial")
