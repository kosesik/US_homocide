...
pred_log_cv <- predict(mod_log_cv, newdata=test_set[-2], type="response")

pred_log_cv_bin<-ifelse(pred_log>=0.5,1,0)

confusionMatrix(as.factor(pred_log__cv_bin),as.factor(test_set$Crime_Solved))

# ROCR curve

roc<-roc(test_set$Crime_Solved~pred_log_cv,plot=TRUE,print.auc=TRUE,col="green",lwd =4,legacy.axes=TRUE,main="ROC Curve - Logistic Regression Cross Validation")
```
