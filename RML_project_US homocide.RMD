...
confusionMatrix(as.factor(pred_log_bin),as.factor(test_set$Crime_Solved))
...
# model_svm<-svm(Crime_Solved~.,data=train_set_balanced)
# 
# summary(model_svm)
# 
...
For the purpose of explainabaility analysis I will focus on the Random Forest model, as it performs better than logistic regression model in terms of sensitivity, i.e. correctly identifying not solved cases from a subset of not solved cases (TP/ TP +FN). On the other hand, differences in overall accuracy are not considerable.
...
temp<-pdp::partial(model_rf, pred.var = x, chull = TRUE)
...
explanation <- lime::explain(d_test[3:5,-2], explainer,n_labels=1, n_features = 6, n_permutations=100)
