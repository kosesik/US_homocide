...
confusionMatrix(as.factor(fitted_results),as.factor(ifelse(test_set$Crime_Solved=="Yes",1,0)))
...
mod_fit <- train(Crime_Solved~.,  data=data, method="glm", family="binomial",
                 trControl = ctrl, tuneLength = 5)
pred <- predict(mod_fit, newdata=test_set[-2])
pred<-ifelse(pred=="Yes",1,0)
confusionMatrix(data=as.factor(pred), as.factor(ifelse(test_set$Crime_Solved=="Yes",1,0)))

roc<-roc(pred,ifelse(test_set$Crime_Solved=="Yes",1,0))
cat("Area under curve :\n", auc(roc))
cat("\n")
plot(roc)
...
model_rf<-randomForest(x=train_set_balanced[-2],
                       y=train_set$Crime_Solved,
                       ntree=10)
pred_rf<-predict(model_rf,newdata=test_set[-2])

confusionMatrix(pred_rf,test_set$Crime_Solved)

roc<-roc(ifelse(pred_rf=="Yes",1,0),ifelse(test_set$Crime_Solved=="Yes",1,0))
cat("Area under curve :\n", auc(roc))
cat("\n")
plot(roc)
