...
data<-subset(data,Victim_Age!=998)
...
#
data$Victim_Age<-cut(data$Victim_Age,breaks=c(0,15,25,35,50,70,100), labels = c("0-15","15-25","25-35","35-50","50-70","70+"),right = FALSE)
...
```{r}
temp<-c("Crime_Solved", "Victim_Age","Perpetrator_Age")
...
fitted_results<-predict(model_log, test_set[-2],type="response")
fitted_results<-ifelse(fitted_results>=0.5,1,0)

length(fitted_results)-sum(fitted_results)
length(fitted_results[fitted_results<0.000000001])

fitted_results[which(fitted_results<0.001)]


confusionMatrix(as.factor(fitted_results),as.factor(test_set$Crime_Solved))
...
roc<-roc(fitted_results,test_set$Crime_Solved)
...
# opt_cp_df<-data.frame(pred=fitted_results, observed=test_set$Crime_Solved)
# 
...
mod_fit <- train(Crime_Solved~.,  data=train_set_balanced, method="glm", family="binomial",
                 trControl = ctrl, tuneLength = 5)
pred <- predict(mod_fit, newdata=test_set[-2])
confusionMatrix(data=as.factor(pred), as.factor(test_set$Crime_Solved))

roc<-roc(pred,test_set$Crime_Solved)
cat("Area under curve :\n", auc(roc))
cat("\n")
plot(roc)
...
model_rf<-randomForest(Crime_Solved~.,data=train_set_balanced, ntree=100 )


pred_rf<-predict(model_rf,newdata=test_set[-2])
# pred_rf<-ifelse(pred_rf=="Yes",1,0)

confusionMatrix(as.factor(pred_rf),as.factor(test_set$Crime_Solved))

roc<-roc(pred_rf,test_set$Crime_Solved)
cat("Area under curve :\n", auc(roc))
cat("\n")
plot(roc)
...
model_svm<-svm(Crime_Solved~.,data=train_set_balanced, type="C-classification", kernel="polynomial")
